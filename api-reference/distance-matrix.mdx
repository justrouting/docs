...
Distance Matrix API returns an N×N matrix of driving times (seconds) and distances (meters) in a single request, powered by the `table` service. Ideal for nearest-store lookup, dispatch, and batch route planning.
...
| `fallback_speed` | float | — | When no route exists, estimate the duration from the as-the-crow-flies distance at this speed |
| `fallback_coordinate` | string | `input` | For fallback estimates, measure from `input` (user-supplied) or `snapped` coordinates |
| `scale_factor` | float | — | Scale all duration values, e.g. `1.2` = everything +20%; use with `annotations=duration` |

<Info>
  Unlike the array-encoded general options, `sources` and `destinations` may contain fewer entries than the number of input coordinates. With `skip_waypoints=true`, both arrays are skipped in the response.
</Info>
...
| `generate_hints` | bool | `true` | Add a `hint` to each response source/destination waypoint for reuse in later requests via `hints` |
...
| `skip_waypoints` | bool | `false` | Remove `sources` and `destinations` from the response (they are still calculated, just not serialized) |
...
| HTTP | code | Meaning |
| --- | --- | --- |
| 400 | `NoTable` | No routable cell in the matrix — use `fallback_speed` as a fallback |
| 400 | `NotImplemented` | The request is not supported |
