...
```
POST https://api.justrouting.tech/vroom
...
<CodeGroup>
```bash curl
curl -X POST "https://api.justrouting.tech/vroom" \
...
There are also `shipments` (pickup-then-delivery pairs) and more advanced fields — the full schema is in the **VROOM (OpenAPI)** section of the API Reference (auto-generated three-panel pages).
