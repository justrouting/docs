...
<Info>
  On success, routing endpoints return HTTP `200` with `code: "Ok"`. Business errors live in the `code` field — don't rely on the HTTP status code alone to distinguish them.
</Info>

## FAQ

<Accordion title="Why does a coordinate in the sea or inside a building still return 200?">
We **snaps** coordinates to the nearest drivable road. `waypoints[].distance` in the response is the offset in meters from your input coordinate to the snapped point — a large value means the input coordinate is low quality.
</Accordion>
