...
Drawing a route on a map takes three steps: request `geometries=geojson`, grab `routes[0].geometry`, hand it to your map library. No polyline decoding needed.
...
- **Fleet Optimization**: with `"options": {"g": true}` in the optimize request, each `routes[i].geometry` in the response is one vehicle's route — color by vehicle
